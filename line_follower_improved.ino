// ADVANCED BIDIRECTIONAL LINE FOLLOWER - IMPROVED VERSION
// Fixes: PID range, cross junction timing, motor ramp reset
// Date: Dec 2, 2025
...
char last_turn = 'r';    // Initialize to 'r' instead of 's'
...
void setup() {
  ...
  if(debug_mode){
    ...
    Serial.println("Version: Improved (Fixed PID & Timing)");
...
void semi_pid(){
  ...
  if(sum == 6 && !just_junction){
    // FIX: Check middle sensors BEFORE moving
    bool middle_line_before = (s[2] || s[3]);
    ...
    if(sum == 6){
      ...
      // FIX: Use pre-movement sensor check
      if(middle_line_before && (s[2] || s[3])){
        // CROSS JUNCTION - GO STRAIGHT
        if(debug_mode) Serial.println("CROSS JUNCTION - GOING STRAIGHT");
        reset_motor_ramp(); // FIX: Reset ramp
        motor(lbase, rbase);
  ...
  if((s[0] && sum <= 2 && !s[5]) || (s[5] && sum <= 2 && !s[0])){
    ...
    reset_motor_ramp(); // FIX: Reset ramp before turn
  ...
  if(sum > 0){
    ...
    // FIX: Reduced PID range to prevent backward motion
    // Old: -150 to +150 (too aggressive)
    // New: -80 to +80 (more reasonable)
    PID = constrain(PID, -80, 80);

    // BIDIRECTIONAL: ALLOW NEGATIVE VALUES (but now limited)
...
void do_turn_left(){
  ...
  motor(0, 0);
  delay(50);
  reset_motor_ramp(); // FIX: Reset ramp after turn
}
